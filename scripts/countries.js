...
function renderCountry(countriesArray) {
  const displayContent = countriesArray
    .map((country) => {
  ...
  mainContainer.innerHTML =
    countriesArray && countriesArray.length > 0 ? displayContent : errorContent;
}

//https://stackoverflow.com/questions/12713564/function-in-javascript-that-can-be-called-only-once
//renderCountry(GITHUB_USERS);

allCountries.onclick = function () {
  renderCountry(COUNTRIES_LIST);
};

asia.onclick = function () {
  const filteredArray = COUNTRIES_LIST.filter(
    (item) => item.continents == "Asia"
  );
  renderCountry(filteredArray);
};

independent.onclick = function () {
  const filteredArray = COUNTRIES_LIST.filter((item) => item.independent);
  renderCountry(filteredArray);
};

population.onclick = function () {
  const filteredArray = COUNTRIES_LIST.filter(
    (item) => item.population >= 50_000_000
  );
  renderCountry(filteredArray);
};

// did i use it correct ? => Yes
document.body.onload = renderCountry(COUNTRIES_LIST);
